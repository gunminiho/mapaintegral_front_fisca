...
import DeletePuntosTacticos from './DeletePuntosTacticos';
...
function UpdPuntosTacticos({ ToggleUpdate }) {
    const [isOpen, setisOpen] = useState(false);
    const dispatch = useDispatch();
    const { puntosTacticos } = useSelector((state) => state.areas);
    ...
    const handleDelete = () => {
        ...
        axios.delete(`${import.meta.env.VITE_APP_ENDPOINT}/puntostacticos/${id}`, id, header())
            .then(response => {
                dispatch(showToast({
                    message: "Punto se elimino correctamente",
                    type: "success"
                }))
                handleClose();
                ToggleUpdate()
            ...
            .catch(error => {
                console.error("Error aliminando el punto tactico:", error);
                dispatch(showToast({
                    message: error.message,
                    type: "error"
                }))
    ...
    const handleUpdate = (values) => {
        formik.setSubmitting(true);
        axios.put(`${import.meta.env.VITE_APP_ENDPOINT}/puntostacticos/${values.id}`, values, header())
            .then(response => {
                dispatch(showToast({
                    message: "Punto tactico actualizado",
                    type: "success"
                }))
                handleClose();
                ToggleUpdate()
            ...
            .catch(error => {
                console.error("Error actualizando el punto tactico:", error.message);
                dispatch(showToast({
                    message: error.message,
                    type: "error"
                }))
    ...
    const SeleccionarPunto = (e) => {
        ...
        const PuntoObj = puntosTacticos.find(punto => punto.id == id)
        formik.setFieldValue('zona', PuntoObj.ZonaAsociada.id)
        formik.setFieldValue('turno', PuntoObj.TurnoAsociado.id)
        formik.setFieldValue('nombre', PuntoObj.nombre)
        formik.setFieldValue('id', PuntoObj.id)
        formik.setFieldValue('puntos', PuntoObj.puntos)
        formik.setFieldValue('nombreFile', 'Archivo.geojson')
    }
    ...
    return (
        <>
            ...
            <CustomModal Open={isOpen} setOpen={setisOpen} handleClose={handleClose}>
                <h1 className='text-lg font-bold'>Editar punto tactico</h1>

                <FormControl size='small' variant='standard' className='w-full' error={formik.errors.id && formik.touched.id}>
                    <InputLabel id="Zona-label">Punto Tactico</InputLabel>
                    <Select
                        labelId="Zona-label"
                        id="Zona"
                    ...
                    >
                        {puntosTacticos?.map((zona, index) => (
                            <MenuItem key={index} value={zona.id}>{zona.nombre} - {zona.ZonaAsociada.nombre} - {zona.TurnoAsociado.nombre}</MenuItem>
                        ))}
                    </Select>
                    <FormHelperText>{formik.errors.turno && formik.touched.turno ? formik.errors.turno : 'Seleccione un punto Tactico'}</FormHelperText>
                </FormControl>
                ...
                <form onSubmit={formik.handleSubmit}>
                    <div className='flex flex-col mt-3 gap-4'>
                        ...
                        <div className='flex justify-between gap-3'>
                            <div>
                                {/* <Button type='button' variant="contained" color="error" onClick={handleDelete}>Eliminar</Button> */}
                                <DeletePuntosTacticos onAccept={handleDelete} idpunto={formik.values.id} />
                            </div>
...
export default UpdPuntosTacticos;
